...
class gamma_data:
    ...
    def read_chn_binary(self):
        ...
        self.start_time_ss = self.infile.read(2).decode('utf-8')  # 解码为字符串
        self.real_time = struct.unpack('I', self.infile.read(4))[0]
        self.live_time = struct.unpack('I', self.infile.read(4))[0]
        self.start_date = self.infile.read(8).decode('utf-8').strip()  # 解码并去除空白
        self.start_time_hhmm = self.infile.read(4).decode('utf-8').strip()  # 解码
        self.chan_offset = struct.unpack('h', self.infile.read(2))[0]
        self.no_channels = struct.unpack('h', self.infile.read(2))[0]    
        self.hist_array = np.zeros(self.no_channels) 
...
if __name__ == '__main__':   
    if len(sys.argv) > 1:
        ...
        filename = input('Filename of binary, (including .Chn): ')
